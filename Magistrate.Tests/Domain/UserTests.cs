	...
	public class UserTests
	{
		...
		public void A_user_must_have_a_key()
		{
			Should.Throw<ArgumentException>(
				() => User.Create(_currentUser, "", "No key user")).Message
		...
		{
			Should.Throw<ArgumentException>(
				() => User.Create(_currentUser, "some-key", "")).Message
		...
		{
			var user = User.Create(_currentUser, "some-key", "some name");
		...
		public void Adding_a_permission_twice_only_keeps_one()
		{
			...
			user.Includes.ShouldBe(new[] { permission.ID });
		...
		public void Removing_a_permission_which_is_not_included_creates_a_revoke()
		{
			...
			user.Includes.ShouldBeEmpty();
			user.Revokes.ShouldBe(new[] { permission.ID });
		}

		[Fact]
		public void Adding_a_permission_which_is_revoked_removes_the_revoke()
		{
			var user = User.Create(_currentUser, "some-key", "some name");
			var permission = Add(Permission.Create(_currentUser, "perm-key", "perm_one", "some description"));
		...
		{
			var user = User.Create(_currentUser, "some-key", "some name");
			var role = Add(Role.Create(_currentUser, "role-key", "role one", ""));
